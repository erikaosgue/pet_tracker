...
$(document).ready(function () {
  ...
  $.ajax('/api/v1/users/' + user_id + '/pets', {
    type: 'GET',
  }).done(function (data) {
    for (const pets of data) {
       $.ajax('/api/v1/pictures/' + pets.id, {
      type: 'GET',
      },
      ).done(result => {
      	console.log("Por fuera", result)
	  ...
	  console.log(result)
        ...
        $('.pet').prepend(
          ...
          <div class="col-xl-9 col-lg-8 col-12 datos d-flex flex-column flex-wrap">
            ...
            <table class="table">
              <tbody>
                <tr class="">
                  <td><b>Raza: </b> <span class="pull-right">${pets.race}</span></td> </tr>
                <tr class="">
                  <td><b>Cumpleaños: </b> <span class="pull-right">${birthday}</span></td> </tr>
                <tr class="">
                  <td><b>Sexo: </b> <span class="pull-right">${pets.sex}</span></td> </tr>
        ...
        $('.button-delete-collar').click((buttonElement) => {
          ...
          if (result) {
            axios({
              method: 'get',
              url: '/api/v1/pets/' + petId1 + '/collars',
              responseType: 'json',
            })
        ...
        function requestApi(user_id, pet_id, collar_id) {
          ...
          $.ajax({
            ...
            success: function (data) {
              if (data === '[]') {
                notyf.error(' collar Id no existe! ');
              } else {
